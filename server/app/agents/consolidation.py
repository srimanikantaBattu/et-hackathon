...
def build_consolidation_agent(db_tools: dict) -> Agent:
    ...
    def get_consolidated_financials(period: str) -> str:
        ...
        log_action("consolidation", f"Consolidated: Revenue ${summary['total_revenue']:,.0f} | EBITDA ${summary['ebitda']:,.0f} ({summary['ebitda_margin_pct']}%)", None, None, "info", db_tools["db"])
