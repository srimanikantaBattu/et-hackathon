...
class MonthEndWorkflowExecutor:
    ...
    def _run_group1_parallel(self) -> None:
        with ThreadPoolExecutor(max_workers=min(8, max(1, len(self.company_ids)))) as pool:
    ...
    def _run_group2_for_company(self, company_id: str) -> None:
        outputs = []
        outputs.append(self._run_agent_with_retry(
    ...
    def _handle_group3_completed(self) -> None:
        ...
        consolidation_output = self._run_agent_with_retry(
            "consolidation",
            build_consolidation_agent,
            None,
            f"Run portfolio consolidation for period {self.period}."
        )
        reporting_output = self._run_agent_with_retry(
